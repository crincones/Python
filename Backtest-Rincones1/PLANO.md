...
| | |
|---|---|
| **Stop inicial** | **150 pontos** da entrada |
| **Parcial** | **50% da posição em +150 pontos** — a mesma distância do stop |
| **No momento da parcial** | **nada a fazer.** O stop já está na média da operação: com meia posição realizada a +150, o stop de −150 é o zero a zero |
...
> configuração (só o setup A, com custo). Ele **mudou** na rodada em que o eixo
> do deslocamento do gatilho virou vaivém — antes dela o stop curto media melhor
> em tudo, e agora não mede mais. Se você preferir fixar um stop por gestão de
> risco em vez de por medição, é `STOP_FIXO` em `plano.py`.


**O stop não anda — nunca.** Ele entra a 150 pontos e fica lá até o trade
acabar. Isso não é descuido: com a parcial saindo na **mesma distância**, o stop
inicial **já é** a média da operação, e o trade que volta depois da parcial morre
em zero. Não há nada para arrastar, e arrastar seria estragar. Mover o stop para
longe continua sendo o erro que transforma uma perda planejada numa perda que não
estava no plano.
...
**Risco por trade** = 150 pontos = **R$ 30,00 por contrato**.

Regra de dimensionamento:

```
contratos = (risco máximo por trade em R$) / R$ 30,00
...
Estes são os números do setup A, stop 150, **descontando 5 pontos de custo por
...
| 4 | O stop entrou a 150 pontos, imediatamente |
| 5 | A parcial saiu a +150 (e o stop ficou onde estava) |
...
perda de 150 pontos, que estava no plano, por uma perda de tamanho
