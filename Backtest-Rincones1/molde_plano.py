...
<div class="cartao">
  ...
  <div class="grade">
    <div class="celula go">
      <div class="k">Setup</div><div class="v">A</div>
      <div class="s">tendência, pullback na média. Só ele.</div>
    </div>
    <div class="celula go">
    ...
    <div class="celula cau">
      ...
      <div class="s">pontos · {risco} por contrato. Nunca se afasta.</div>
    </div>
    <div class="celula go">
...
""".format(stop=C['stop'], parcial=C['parcial'], alvo=C['alvo'],
    ...
    chips = ''.join(
        '<span class="chip">%s <b>%s</b></span>' % (k, v) for k, v in (
            ...
            ('stop', '%d pts' % C['stop']),
            ('alvo', '+%d pts' % C['alvo']),
            ('trades/pregão', '~%.1f' % fu['por_pregao']),
