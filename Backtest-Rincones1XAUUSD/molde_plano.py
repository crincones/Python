...
def render(C, md):
    # `ps` e a psicologia da configuracao do plano na amostra inteira
    ps = C['ps']
...
<div class="cartao">
  ...
  <div class="grade">
    <div class="celula go">
      <div class="k">Gatilho</div><div class="v">{gatilho}</div>
      <div class="s">{gatilho_s}</div>
    </div>
    <div class="celula go">
      <div class="k">Entrada</div><div class="v">a mercado</div>
      <div class="s">no fechamento da barra do gatilho. Nunca limitada.</div>
    </div>
    <div class="celula cau">
      <div class="k">Stop</div><div class="v">{stop}</div>
      <div class="s">USD · {risco} por lote de 100 oz. Nunca se afasta.</div>
    </div>
    <div class="celula go">
      <div class="k">Parcial</div><div class="v">+{parcial}</div>
      <div class="s">{frac}% da posição, e o resto vai a zero a zero.</div>
    </div>
    <div class="celula go">
      <div class="k">Alvo</div><div class="v">+{alvo}</div>
      <div class="s">USD, no restante da posição.</div>
    </div>
...
""".format(stop=C['stop'], parcial=C['parcial'], alvo=C['alvo'],
           frac=C['frac'], risco=C['risco_rs'],
           gatilho=C['gatilho'], gatilho_s=C['gatilho_s'])

    chips = ''.join(
        '<span class="chip">%s <b>%s</b></span>' % (k, v) for k, v in (
            ('ativo', 'XAUUSD'), ('gráfico', '521 ticks'),
            ('gatilho', C['gatilho']),
            ('stop', '%s USD' % C['stop']),
            ('alvo', '+%s USD' % C['alvo']),
            ('trades/pregão', ('~%.1f' % ps['por_pregao']).replace('.', ',')),
            ('pregões positivos', '%d%%' % round(ps['dias_pos'])),
        ))

    return """%s
<div class="wrap">
<header class="topo">
  <p class="eyebrow">Rincones1 · procedimento operacional</p>
  <h1>Plano operacional<br>XAUUSD · 521 ticks</h1>
...
<footer>
  ...
  Números descontando %s USD de custo por trade.
</footer>
</div>
""" % (CABECA, chips, corpo, cartao, C['custo'])
