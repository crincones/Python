...
Regras da construcao:

  * a caixa mede `renko_box_ticks` ticks do ativo (USDJPY: 50 x 0.001 = 0.050);
  * a grade e ancorada em ZERO -- todas as aberturas e fechamentos de tijolo
    sao multiplos exatos da caixa;
...
def box_from_ticks(ticks: int, tick_size: float) -> float:
    """Caixa em unidades de preco a partir do tamanho em ticks (pontos do MT5).

    Aqui a conversao e direta -- N ticks valem N * tick_size. A convencao "NR =
    (N-1) ticks" do ProfitChart nao se aplica ao MetaTrader, que nao tem esse
    tipo de grafico embutido.
    """
