...
def main(argv=None) -> int:
    ...
    cfg = replace(Config(), csv_path=a.csv, cluster_method=a.metodo, top_n=a.top,
                  out_dir=a.out, cache=False, mode=a.modo,
    ...
    for rule in ("15min", "1h", "1D"):
        ...
        print(f"2) ATR {rule:>5}: mediano={a_.median():8.1f} pts | "
              f"p10={a_.quantile(.10):7.1f} | p90={a_.quantile(.90):7.1f} "
              f"| barras={len(bars):,}")
    ...
    if res.detection.bricks is not None:
        ...
        print(f"   Renko {cfg.renko_box_ticks}R = {res.detection.box:g} pts | "
    ...
    print(f"5) clustering ({cfg.cluster_method}, espaco {res.info['space']}): "
          f"{res.info['n_clusters']} clusters | banda={res.info['band_pts']:.0f} pts | "
    ...
    frame = pd.DataFrame([{
        ...
        "reacao_media": round(lv.mean_reaction),
